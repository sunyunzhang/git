...
class playGame extends Phaser.Scene {
    ...
    update() {
        if (num == 2) {
            this.showHand(260, 650)
            num = 0
        }
        if (num == 3) {
            this.showHand(280, 650)
            num = 0
        }
        if (num == 4) {
            this.showHand(450, 450)
            num = 0
        }
        if (num == 5) {
            this.showHand(310, 330)
            num = 0
        }
    }
    ...
    showHand(x, y) {
        ...
        this.input.on('pointerdown', () => {
            ...
            clearInterval(this.huan_interval)
        }, this)
    }
    //枕头
    showPallow() {
        this.pallow.on('pointerdown', () => {
            this.tweens.add({
                targets: [this.pallow],
                x: this.pallow.x + 150,
                duration: 500,
